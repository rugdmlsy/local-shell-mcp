...
The controller issues a capability with `POST /api/control/sessions/{session_id}/capabilities`. Issuance rotates a persisted per-Session generation, so concurrent requests leave at most one valid capability. Cleanup and terminal lifecycle requests fence future issuance before stopping resources. An MCP request sends `X-LSM-Session-Capability`; when capability enforcement is enabled, a missing or revoked capability is rejected. The bound Agent must pass the pinned `logical_session_id` for ordinary tools. A different ID, or a Job/shell owned by a different Session, is rejected. The bound Agent may only use `session_manage(get/report)` and cannot manage LSM Goal Plans or global LSM services.
